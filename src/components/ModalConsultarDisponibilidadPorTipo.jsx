// Modal para consultar disponibilidad por tipo de profesional
import React, { useState } from 'react';
import { useEmpresasYTipos } from './profesionales/modals/useEmpresasYTipos';
import { useEmpresa } from '../EmpresaContext';

const ConsultarDisponibilidadPorTipoModal = ({ show, onClose }) => {
  ...
  const handleConsultar = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/profesionales/por-tipo?tipoProfesional=${encodeURIComponent(tipoProfesional)}&empresaId=${empresaSeleccionada.id}`
      );
      if (!response.ok) {
      ...
      <div className="modal-dialog" style={{marginTop: '120px', maxWidth: '700px', width: '99vw'}}>
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Consultar disponibilidad por tipo de profesional</h5>
          ...
          <div className="modal-body" style={{padding: '18px 12px 12px 12px'}}>
            ...
            <div className="mt-3">
              ...
              {resultados && Array.isArray(resultados) && resultados.length > 0 && (
                <div>
                  <h5>Profesionales disponibles:</h5>
                  <ul>
                    {resultados.map(r => (
                      <li key={r.id}>
                        ...
                        Obras asignadas: {r.cantidadObrasAsignadas} | Honorarios: {r.cantidadHonorarios}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {resultados && Array.isArray(resultados) && resultados.length === 0 && (
                <div className="alert alert-info">No se encontraron profesionales para los criterios ingresados.</div>
              )}
            </div>
          </div>
        </div>
...
export default ConsultarDisponibilidadPorTipoModal;
